    ...
    async def ask_human_expert(self, context: RunContext, question_text: str):
        ...
        while True:
            if asyncio.get_event_loop().time() > timeout_time:
                logger.warning(
                    f"Timeout waiting for human expert response for question {question_id}"
                )
                try:
                    async with httpx.AsyncClient() as client:
                        await client.put(
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    if data.get("status") == "answered":
                        answer = data.get(
                            "answer_text",
                            "I'm sorry, I couldn't get an answer from my supervisor.",
                        )
...
async def entrypoint(ctx: JobContext):
    ...
    if knowledge_base_entries:
        ...
        for item in knowledge_base_entries:
            knowledge_string += (
                f"- Q: {item['question_text']} A: {item['answer_text']}\n"
            )

    # The user's base instructions

    base_instructions = """You are a helpful AI receptionist for 'Hyderabad Salon'.
                        You are polite and professional.
                        You MUST follow these rules:
                        1. You know the salon's hours: Tuesday-Sunday, 9 AM to 6 PM.
                        2. You know the salon's location: Road No.1, Jubilee Hills.
                        3. You know what services are offered: haircuts, coloring, styling, pedicure and manicure.
                        4. You DO NOT know any pricing.
                        5. If you are asked about anything you know (hours, location, services), answer it.
    ...
    agent = Assistant(
        instructions=full_instructions, knowledge_base_entries=knowledge_base_entries
    )
